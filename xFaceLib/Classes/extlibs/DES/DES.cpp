...
#define ENCRYPT 1
#define DECRYPT 2
#define extend (size)  ((size/8+1)*8);

static void des(unsigned char* in, unsigned char* out);
static void des_block(unsigned char* in, unsigned char* out);

static void deskey(unsigned char key[8], int md);
...
// ----------------------------------------------------------------------
static void deskey(unsigned char key[8], int md)  /* Thanks to James Gillogly &amp; Phil Karn! */
{
...
int XDes_TripleEncrypt(char key[16],char *indata,char*outdata)
{
    ...
    ret = XDes_Encrypt(key,indata,outdata);
    ret = XDes_Decrypt(key+8,outdata,outdata);
    ret = XDes_Encrypt(key,outdata,outdata);
...
int XDes_TripleDecrypt(char key[16],char *indata,char *outdata)
{
    ...
    ret = XDes_Decrypt(key,indata,outdata);
    ret = XDes_Encrypt(key+8,outdata,outdata);
    ret = XDes_Decrypt(key,outdata,outdata);
...
int XDes_Encrypt ( char key[8], char*indata,char *outdata)
{
   ...
   des ( (unsigned char*)indata, (unsigned char*)outdata);
...
{
	if (type) {
		XDes_TripleDecrypt(key,indata,outdata);
	}else{
		XDes_Decrypt(key,indata,outdata);
	}
	return 1;
}
...
{
	if (type) {
		XDes_TripleEncrypt(key,indata,outdata);
	}else{
		XDes_Encrypt(key,indata,outdata);
	}
	return 1;
}
